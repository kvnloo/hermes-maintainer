# Notes for agents — hermes-maintainer
...
- Python package: `src/hermes_maintainer/` except `ui/`
- Dashboard / xyflow canvas: `src/hermes_maintainer/ui/` — do not revert unrelated UI work in flight
...
| Frontend | `npm test --prefix src/hermes_maintainer/ui/graph-src` (when `scripts.test` exists) |
| Mutation | `mutmut run` |
| Runtime | `hermes-maintainer serve` then open `http://127.0.0.1:8766` |
| Pages | `python3 scripts/build-pages.py` |
